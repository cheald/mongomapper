...
describe "Support" do
  ...
  context "Time.to_mongo without Time.zone" do
    ...
    it "should be time to milliseconds if string" do
      Time.to_mongo('2000-01-01 01:01:01.123456').to_f.should be_within(0.000001).of(Time.local(2000, 1, 1, 1, 1, 1, 123000).utc.to_f)
    end
  ...
  context "Time.to_mongo with Time.zone" do
    it "should be time to milliseconds if time" do
      Time.zone = 'Hawaii'
      Time.to_mongo(Time.zone.local(2009, 8, 15, 14, 0, 0, 123456)).to_f.should be_within(0.000001).of(Time.utc(2009, 8, 16, 0, 0, 0, 123000).to_f)
      Time.zone = nil
    end
    ...
    it "should not round up times at the end of the month" do
      t = Time.now
      Time.to_mongo(t.end_of_month).tv_sec.should == t.end_of_month.utc.tv_sec
    end
